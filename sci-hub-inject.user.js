...
function pubMed() {
  ...
  <a class="link-item dialog-focus" href="${sciHubLink(
    doi
  )}" target="_blank" rel="noopener" title="See full text on SciHub"><img src="${config['icon']}" alt="full text provider logo" width="24" height="24"><span>Sci-Hub</span></a>
  `;
}
  ...
  menu.innerHTML += `
	<div class="js-access-button">
			<a href="${sciHubLink(doi)}" class="c-article__button">
				<img class="u-icon" width="18" height="18" aria-hidden="true" src="${config['icon']}" width=24 height=24 alt="Sci-Hub logo"/>
...
function sciencedirect() {
  ...
  setTimeout(() => {
    ...
    <a
      ...
      href="${sciHubLink(doi)}"
      >
      <img src="${config['icon']}" width=24 height=24 alt="Sci-Hub logo" /> 
...
function science() {
  ...
  <a
    ...
    ><img src="${config['icon']}" width=24 height=24 class="mr-2" alt="Sci-Hub logo" /> <span>get free access</span></a
  >
  `;
}
...
function wiley() {
  ...
  <div class="coolBar__section coolBar--download PdfLink cloned">
    ...
    <img src="${config['icon']}" width=24 height=24 alt="Sci-Hub logo" />Access on SciHub
    </a>
  </div>
  `;
}
...
function springerLinkArticle(doi) {
  ...
  if (details) {
    ...
    <a class="c-article-info-details__cite-as" href="${sciHubLink(doi)}" title="SciHub">
      <img src="${config['icon']}" width=24 height=24 style="vertical-align:bottom" alt="Sci-Hub logo"/>
      View On SciHub
    </a>
  `;
  }
  ...
  contextContainer.innerHTML += `
    <div style="align-self:center">
      <a href="${sciHubLink(doi)}" title="SciHub">
        <img src="${config['icon']}" width=24 height=24 style="width:24px; vertical-align:bottom" alt="Sci-Hub logo"/>
        View On SciHub
      </a>
    </div>
  `;
